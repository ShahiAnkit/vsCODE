...
class ProductController extends Controller
{
                        ...
                        <table style="width: 100%">';
                            if(!empty($keyword->keywordProduct()->get())){
                                foreach ($keyword->keywordProduct()->get() as $product){
									$statusClass="";
									if ($product->product()->isAllowed == "Approved")
									{
                                    ...
                                    $table .='<tr>
                                        <td style="width: 5%">'.$product->product()->id.'</td>
                                        <td style="width: 15%">'.$product->product()->name.'</td>
                                        <td  style="width: 15%">'.$product->product()->category.'</td>
										<td class="'.$statusClass.'" style="width: 5%">'.$product->product()->isAllowed.'</td>
										<td style="width: 10%">'.$product->product()->Remarks.'</td>
                                        <td style="width: 25%">'.$product->product()->Manufacturer.'</td>
										<td style="width: 25%">'.$product->product()->Source.'</td>
    ...
    public function indexJson(Request $request)
    {
        ...
        return DataTables::of($Product)
            ...
            ->editColumn('isAllowed', function ($data) {
                return (!empty($data->isAllowed)) ? $data->isAllowed : "-";
            })
            ->editColumn('Source', function ($data) {
                return (!empty($data->Source)) ? $data->Source : "-";
            })
			->editColumn('Manufacturer', function ($data) {
                return (!empty($data->Manufacturer)) ? $data->Manufacturer : "-";
            })
			->editColumn('Remarks', function ($data) {
                return (!empty($data->Remarks)) ? $data->Remarks : "-";
            })
            ->rawColumns(['id','name', 'category', 'price','isAllowed','Source','Manufacturer','Remarks','created_at'])
            ->make(true);
    }
